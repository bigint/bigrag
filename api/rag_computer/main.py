...
from rag_computer import __version__
from rag_computer import config as config_module
from rag_computer import db as db_module
from rag_computer.config import Settings
from rag_computer.db.bootstrap import run_migrations
from rag_computer.exceptions import (
    ForbiddenError,
    NotFoundError,
    ServerError,
    UpstreamError,
...
from rag_computer.logging import RequestLoggingMiddleware, configure_logging, get_logger
from rag_computer.middleware.cors import RuntimeCorsMiddleware
from rag_computer.middleware.csrf import SessionCsrfMiddleware
from rag_computer.middleware.idempotency import IdempotencyMiddleware
from rag_computer.middleware.maintenance import MaintenanceWriteLockMiddleware
from rag_computer.services import crypto, redis_cache, runtime_settings
from rag_computer.services.access_log import AccessLogMiddleware
from rag_computer.services.event_bus import event_bus
from rag_computer.services.queue import ingestion_queue
from rag_computer.services.storage import init_storage_from_runtime
from rag_computer.services.vector_store import vector_store
from rag_computer.services.webhook import WebhookDispatcher
from rag_computer.startup_guard import check_production_safety

_CLI_CONFIG_PATH_ENV = "_RAG_COMPUTER_CLI_CONFIG_PATH"
_CLI_OVERRIDES_ENV = "_RAG_COMPUTER_CLI_OVERRIDES"
...
async def lifespan(app: FastAPI):
    ...
    logger = get_logger("rag-computer")
    ...
    if not s.cors_origins and s.env != "dev":
        logger.warning(
            "RAG_COMPUTER_CORS_ORIGINS is empty — every cross-origin browser request will "
    ...
    if not crypto.is_configured():
        logger.warning(
            "RAG_COMPUTER_MASTER_KEY not set — provider credentials will be stored "
    ...
    from rag_computer.services.google_drive import google_drive_scheduler

    await google_drive_scheduler.start()
    app.state.google_drive_scheduler = google_drive_scheduler

    from rag_computer.services.cleanup import cleanup_old_data
...
def create_app(settings_override: Settings | None = None) -> FastAPI:
    ...
    from rag_computer.routers.admin_access import router as admin_access_router
    from rag_computer.routers.admin_api_keys import router as admin_api_keys_router
    from rag_computer.routers.admin_audit import router as admin_audit_router
    from rag_computer.routers.admin_backups import router as admin_backups_router
    from rag_computer.routers.admin_connectors import router as admin_connectors_router
    from rag_computer.routers.admin_realtime import router as admin_realtime_router
    from rag_computer.routers.admin_settings import router as admin_settings_router
    from rag_computer.routers.admin_users import router as admin_users_router
    from rag_computer.routers.auth import router as auth_router
    from rag_computer.routers.chat import router as chat_router
    from rag_computer.routers.collections import router as collections_router
    from rag_computer.routers.connectors import router as connectors_router
    from rag_computer.routers.documents import global_router as documents_global_router
    from rag_computer.routers.documents import router as documents_router
    from rag_computer.routers.embedding_presets import router as embedding_presets_router
    from rag_computer.routers.evaluation import router as evaluation_router
    from rag_computer.routers.health import router as health_router
    from rag_computer.routers.mcp_servers import router as mcp_servers_router
    from rag_computer.routers.preferences import router as preferences_router
    from rag_computer.routers.query import router as query_router
    from rag_computer.routers.upload_sessions import router as upload_sessions_router
    from rag_computer.routers.usage import router as usage_router
    from rag_computer.routers.webhooks import router as webhooks_router
    ...
    from rag_computer.services.mcp_http import build_mcp_http_app
...
def cli() -> None:
    parser = argparse.ArgumentParser(description="rag.computer server")
    parser.add_argument("--config", default="rag-computer.toml", help="Config file path")
