...
logger = get_logger("rag_computer.webhook")
...
class WebhookDispatcher:
    ...
    async def _get_webhooks(self) -> list[dict]:
        ...
        from rag_computer.db.models import Webhook
    ...
    async def _get_collection_for_document(self, document_id: str) -> str | None:
        ...
        from rag_computer.db.engine import session_factory
        from rag_computer.db.models import Collection, Document
    ...
    async def _deliver(self, webhook: dict, event: str, payload: str) -> None:
        ...
        from rag_computer.db.engine import session_factory
        from rag_computer.db.models import WebhookDelivery
        ...
        async with sem:
            ...
            headers = {
                ...
                "X-Rag-Computer-Signature": signature,
                "X-Rag-Computer-Event": event,
                "X-Rag-Computer-Delivery": str(delivery_id),
                "User-Agent": "rag-computer-webhooks/1.0",
            }
            ...
            for attempt in range(1, len(retry_delays) + 2):
                ...
                try:
                    from rag_computer.models.webhook import resolve_and_validate_url
    ...
    async def deliver_once(self, webhook: dict, event: str, payload: str) -> dict:
        ...
        headers = {
            ...
            "X-Rag-Computer-Signature": signature,
            "X-Rag-Computer-Event": event,
            "X-Rag-Computer-Delivery": str(uuid.uuid4()),
            "User-Agent": "rag-computer-webhooks/1.0",
        }
        try:
    ...
    async def deliver_test(self, webhook: dict) -> dict:
        ...
        headers = {
            "Content-Type": "application/json",
            "X-Rag-Computer-Signature": signature,
            "X-Rag-Computer-Event": "webhook.test",
            "X-Rag-Computer-Delivery": str(uuid.uuid4()),
            "User-Agent": "rag-computer-webhooks/1.0",
        }
