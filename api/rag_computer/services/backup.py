...
from rag_computer.db.models import AuditLog, BackupJob, Collection, Document, EmbeddingCache
from rag_computer.logging import get_logger
from rag_computer.services.maintenance import acquire_backup_lock, release_backup_lock
from rag_computer.services.queue import ingestion_queue
from rag_computer.services.runtime_settings import all_runtime_values
from rag_computer.services.storage import get_storage
from rag_computer.services.vector_store import vector_store

logger = get_logger("rag_computer.backup")
...
async def _run_locked_backup(job_id: uuid.UUID) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=f"rag-computer-backup-{job_id}-") as raw_dir:
