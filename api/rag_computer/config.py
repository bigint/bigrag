...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="RAG_COMPUTER_", env_nested_delimiter="__")
    ...
    database_url: str = (
        "postgres://rag_computer:rag_computer@localhost:5432/rag_computer?sslmode=disable"
    )
    db_pool_min: int = 5
    db_pool_max: int = 50
    ...
    session_cookie_name: str = "rag_computer_session"
        ...
        for section, values in data.items():
            if isinstance(values, dict):
                for k, v in values.items():
                    key = f"{section}_{k}"
                    if f"RAG_COMPUTER_{key.upper()}" not in os.environ:
                        flat[key] = v
            else:
                if f"RAG_COMPUTER_{section.upper()}" not in os.environ:
                    flat[section] = values
