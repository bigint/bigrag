...
Mirrors the schema built by the legacy MIGRATIONS array exactly. Structural
changes go through Alembic — do not edit this file without generating a
migration.

Note: the ``metadata`` column on several tables is mapped to a Python
attribute named ``meta`` because ``metadata`` is reserved on DeclarativeBase.
The column name in Postgres remains ``metadata``.
"""
