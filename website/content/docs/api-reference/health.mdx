...
Returns HTTP `503` with `"status": "degraded"` when any dependency is unhealthy. Embedding errors are categorized as `auth_failed`, `rate_limited`, `timeout`, `unreachable`, or `unknown`. The embedding health check result is cached for 60 seconds.
...
| `413` | Payload Too Large | File exceeds `RAG_COMPUTER_MAX_UPLOAD_SIZE_MB` |
