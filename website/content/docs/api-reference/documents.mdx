...
`.tar` archives are automatically extracted and all supported files inside are ingested individually. When `file_types` is set, only matching files inside tars are extracted.
