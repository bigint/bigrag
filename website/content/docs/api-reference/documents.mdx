...
```
POST /v1/collections/{collection_name}/documents/url
```

Fetch a URL and ingest its content as a document. Supports HTML pages, PDFs, and any other supported file type served over HTTP.

```bash
curl -X POST http://localhost:6100/v1/collections/research/documents/url \
  -H "Authorization: Bearer $BIGRAG_API_SECRET" \
  -H "Content-Type: application/json" \
  -d '{"url": "https://example.com/report.pdf", "metadata": {"source": "web"}}'
```

| Field | Type | Required | Notes |
|-------|------|----------|-------|
| `url` | string | yes | HTTP or HTTPS URL to fetch |
| `metadata` | object | no | Additional metadata (merged with `source` and `source_url`) |

The file extension is inferred from the URL path or the response `Content-Type` header. The document's metadata will include `source: "url"` and `source_url` automatically.

**Response** `201`: Document object (same as file upload).

**Errors:** `400` — Invalid URL, fetch failed, or empty content. `413` — Content too large.
...
List objects in an S3 bucket (optionally filtered by prefix) and ingest all supported files. Max 100 files per request.
...
```json
{
  "status": "ok",
  "documents": [
    { "id": "...", "filename": "report.pdf", "status": "pending", "..." : "..." }
  ],
  "total": 5,
  "skipped": ["reports/2026/readme.txt.bak"]
}
```

**Errors:** `400` — S3 connection failed, no supported files found, or too many files (>100).
...
Stream aggregated progress for multiple documents via SSE. Useful after URL or S3 ingestion to track all documents in a single stream.
