...
| `document:read` | List / get / download documents, including `/v1/collections/{name}/documents/{id}`, `/chunks`, and `/file`, plus batch get/status, upload-session status, and global `/v1/documents/{id}` lookups |
...
| `/v1/admin/api-keys`, `/v1/admin/users`, `/v1/admin/settings`, `/v1/admin/backups`, `/v1/admin/connectors`, `/v1/admin/webhooks`, `/v1/admin/embedding-presets`, `/v1/admin/mcp-servers` | ✓ | — | — |
