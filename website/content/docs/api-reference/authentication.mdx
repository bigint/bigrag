...
Collection SSE still accepts long-lived API keys in `?token=` for older clients, but that form is deprecated because URL tokens can land in proxy logs. New EventSource clients should mint a short-lived event token first.
