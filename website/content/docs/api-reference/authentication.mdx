...
<Tabs items={["Header", "Query (collection SSE only)"]}>
<Tab value="Header">
```bash
...
<Tab value="Query (collection SSE only)">
```bash
# For EventSource / SSE clients that can't set headers. Only collection
# event paths ending in /events accept ?token= — every other endpoint requires
