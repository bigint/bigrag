...
Send the key as a bearer token. For browser WebSocket clients that cannot set bearer headers, mint a short-lived collection realtime token first.

<Tabs items={["Header", "Realtime token"]}>
<Tab value="Header">
```bash
...
<Tab value="Realtime token">
```bash
REALTIME_TOKEN=$(curl -s -X POST http://localhost:4000/v1/collections/docs/realtime-token \
  -H "Authorization: Bearer bigrag_sk_…" | jq -r .token)
```
</Tab>
</Tabs>

The `collection.events` realtime topic accepts bearer headers or short-lived realtime tokens. Long-lived API keys are not accepted in query strings or WebSocket message params.
...
| `WS /v1/realtime` collection topics, `POST /v1/collections/{name}/realtime-token` | ✓ | ✓ | — |
...
| `/v1/admin/access`, `/v1/admin/audit`, `WS /v1/realtime` admin topics, `/v1/auth/preferences` | ✓ | — | — |
