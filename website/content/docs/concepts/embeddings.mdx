...
rag.computer supports four provider families. Each collection picks one at creation time and stores its own copy of the config, so different collections on the same instance can use different models.
...
4. New vectors are batched at `RAG_COMPUTER_INGESTION_BATCH_SIZE` and written to the configured vector store.
5. Queries embed the same way, with short-lived Redis caching for repeated query embeddings.

The cache key is `(content_hash, provider, model, dimension)`. Switching a collection's model invalidates cache hits automatically — on re-embed, unchanged chunks under the **new** model still start cold, but if you switch back they're hot again.

Persistent embedding-cache rows are encrypted by default with `RAG_COMPUTER_MASTER_KEY`. Set `embedding_cache_mode` to `disabled` in `/settings` when you prefer provider cost over storing reusable vectors. The default retention window is 30 days after last use, and admins can purge the cache from the Security settings tab.

Redis query caches are also encrypted when `RAG_COMPUTER_MASTER_KEY` is configured. The default query embedding TTL is 300 seconds; set `query_embedding_cache_ttl` to `0` to disable it.

## Concurrency & throughput

Embedding requests are guarded by a semaphore (`RAG_COMPUTER_EMBEDDING_CONCURRENCY`, default 8). Raise it for high-QPS providers; lower it if your embedding provider throttles requests. Token counting uses `tiktoken` where the provider ships a tokenizer and falls back to a 4-character-per-token heuristic otherwise.
...
<Callout type="warn">
The embedding provider, model, dimension, and `embedding_base_url` **cannot** be changed after a collection is created — chunk vectors are only meaningful under the model that produced them. If you need to switch models, create a new collection, trigger [`POST /v1/collections/{name}/reembed`](/docs/api-reference/collections#re-embed-collection) after updating, or run a re-ingest.
</Callout>
