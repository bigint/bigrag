...
- **Redis** — the ingestion queue (`BLPOP/BLMOVE` semantics),
  delayed retry set, processing leases, idempotency-key cache,
  auth/collection/query hot-path caches, and short-lived
...
message. Active workers renew Redis leases while long jobs run, failed
jobs wait in a delayed retry set before the next attempt, and exhausted
jobs are retained in the dead-letter list for operator visibility.
...
- **Redis persistence**: enable `appendonly yes` (already set in the
  shipped compose file). Pending jobs, delayed retries, processing
  leases, and dead-letter entries survive process restarts within the
  Redis durability window.
...
ingestion queue. Scheduled connector ticks claim due sources with row
locks so multiple API replicas do not create duplicate scheduled sync
jobs. Provider modules layer on top of those shared helpers; the Google
