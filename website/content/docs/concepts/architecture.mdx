...
- **Turbopuffer search** — dense embeddings, chunk text, payload
  metadata, BM25 full-text search, filtered semantic search, and
...
Turbopuffer provides the managed vector and full-text search backend.
</Callout>
...
- **Dramatiq workers**: run one or more `bigrag-worker` processes for
  ...
  before reserving jobs, aborts boot if initialization fails, refreshes
  queue-scoped heartbeats while idle, and recovers stale processing
  leases on startup. Periodic maintenance and webhook outbox ticks are
