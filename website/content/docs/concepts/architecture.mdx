...
rag.computer intentionally keeps four distinct stores instead of shoving
...
<Callout type="info">
rag.computer can run entirely from `docker compose up` on a single machine
...
- **Unstructured.io** — great coverage but requires network calls and
  a hosted inference server by default. rag.computer needs to run offline.
...
- **Vector engine**: Qdrant gives rag.computer a dedicated HNSW vector
...
— and the `VectorStore` abstraction in rag.computer leaves a door open for
