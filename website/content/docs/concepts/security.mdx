...
rag.computer's auth model has two credential types, one scope system, and an immutable audit log. This page is the high-level map; the API shapes live under [API Reference → Authentication](/docs/api-reference/authentication).

## The two credentials

| Credential | Used by | Lifetime | Revocation |
|------------|---------|----------|-----------|
| **Session cookie** | admin UI, browser flows | `RAG_COMPUTER_SESSION_EXPIRY_HOURS` (default 7 days) | `/v1/auth/logout`, `/v1/auth/logout-all`, or cleanup sweeper |
| **API key** (`ragc_sk_…`) | SDKs, backend services, CI | Indefinite unless `expires_at` set | `DELETE /v1/admin/api-keys/{id}` or `active: false` |
...
Session-cookie mutating requests must include an `Origin` header that matches the API origin or one of the configured `RAG_COMPUTER_CORS_ORIGINS`. This protects the direct-CORS admin UI from cross-site request forgery while still allowing API-key clients to use normal bearer authentication.
...
document:upload          # POST /v1/collections/{name}/documents, batch upload, reprocess
document:read            # GET documents, chunks, download file, batch get/status, /v1/documents/{id}
...
Run with `RAG_COMPUTER_ENV=prod` — the server refuses to boot if:

- `RAG_COMPUTER_SESSION_COOKIE_SECURE` is `false`
- `RAG_COMPUTER_HOST` binds publicly without `RAG_COMPUTER_ALLOW_PUBLIC_BIND_IN_PROD=true`
- `RAG_COMPUTER_SESSION_COOKIE_DOMAIN` is set without trusted proxies
- `RAG_COMPUTER_MASTER_KEY` is unset
- Postgres is still using the shipped `rag_computer:rag_computer` default password
