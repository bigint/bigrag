...
Scopes are `resource:action` pairs. Full-access keys are stored with `["*:*"]`; legacy keys without `scopes` are also treated as full access. Any narrower scope list is restricted to the union of those patterns. `*` is a wildcard.
...
Collections created with `tenant_field` require that field in upload metadata and raw vector metadata. Tenant-scoped API keys get tenant filters applied automatically on query, chat, document listing, and document reads. Session admins can use explicit tenant filters. Unscoped API keys without a tenant are denied with `400` before tenant-guarded retrieval.
