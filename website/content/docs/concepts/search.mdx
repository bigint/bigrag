...
Multiple filters are combined with AND. When a collection was created with `tenant_field`, bigRAG creates a Qdrant payload index for that field and requires the tenant field in every query and chat filter. Missing tenant filters return `400`.
