...
When a query-result cache entry is reused, `cache_hit` is `true`, `cache_ms` contains the Redis lookup time, and the embed/search/rerank timings stay at zero instead of replaying the original uncached request's latencies.
