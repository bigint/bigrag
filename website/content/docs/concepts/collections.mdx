...
| `vector_store_provider` | string | no | `"qdrant"` | `qdrant` or `turbopuffer`; fixed after creation. `turbopuffer` requires a saved turbopuffer API key in Vector Storage before creation |
