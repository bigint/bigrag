...
Set `multimodal_enrichment_enabled: true` to queue asynchronous LLM summaries for image, table, and equation elements. Enrichment reuses the collection's own embedding key and runs only for collections on the `openai` provider; collections using `openai_compatible`, Cohere, or Voyage are not enriched. Core document readiness is still based on parse/chunk/embed completion; enrichment can finish later or fail independently.
