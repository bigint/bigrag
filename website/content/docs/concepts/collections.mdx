...
A **collection** is a logical grouping of documents that share the same embedding configuration. Each collection maps to a Qdrant collection for vector storage.
...
| `embedding_provider` | string | no | Server default | `openai`, `cohere`, or `voyage` |
