---
title: Quickstart
description: Upload and search your first document with rag.computer in under 5 minutes.
---
...
The response sets a `rag_computer_session` cookie. The admin UI exposes the same flow at `http://localhost:3000/setup`.
</Step>
...
export RAG_COMPUTER_API_KEY=$(jq -r .key /tmp/api-key.json)
```

All further examples use `$RAG_COMPUTER_API_KEY` as a bearer token. See [Authentication](/docs/api-reference/authentication) for scopes.
</Step>
...
const client = new RagComputer({
  apiKey: process.env.RAG_COMPUTER_API_KEY!,
