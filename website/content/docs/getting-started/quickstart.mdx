...
The response sets a `bigrag_session` cookie. The admin UI exposes the same flow at `http://localhost:3000/setup`, then redirects to `/onboarding` where you create one verified embedding preset and save Turbopuffer settings before collection work.
</Step>

<Step>
### Complete provider onboarding

In the admin UI, finish `/onboarding` by adding one embedding preset and saving Turbopuffer connection details. You can skip Turbopuffer during onboarding for a read-only tour, but collections cannot ingest or query until the backend has a working Turbopuffer connection.
</Step>
...
- Open the [admin UI](/docs/admin-ui) at `http://localhost:3000` for a visual view of collections, documents, and query analytics.
