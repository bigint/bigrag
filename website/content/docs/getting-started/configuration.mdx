...
The same settings are available through [`/v1/admin/settings`](/docs/api-reference/instance-settings). Database-backed admin settings apply immediately on save. Turbopuffer connection changes validate the new target before saving, then swap the runtime client in place where applicable.
