...
| `BIGRAG_VECTOR_STORE_PROVIDER` | Vector backend: `qdrant` or `turbopuffer`. | `qdrant` |
...
| Vector Storage (`/vector-storage`) | Qdrant or turbopuffer provider, Qdrant URL and readiness behavior, turbopuffer API key, and turbopuffer region. |
| Backups | S3-compatible readable backup bucket, endpoint, region, prefix, and credentials for S3, R2, or MinIO. |

The UI shows common controls first and keeps raw setting keys, source metadata, and rarely changed limits in Advanced controls. Legacy Settings deep links such as `/settings?tab=storage` still open the relevant Settings panel, `/settings?tab=vector_store` redirects to `/vector-storage`, and `/settings?tab=models`, `/settings?tab=search`, and `/settings?tab=chat` redirect to `/models?tab=settings`.

Database-backed admin settings apply immediately on save. Storage, backup, and vector-store changes validate the new target before saving, then swap the active backend in place where applicable.
