...
After the first admin account exists, go to `/settings` to manage platform runtime settings and `/models` to manage model presets and model runtime settings. Runtime settings are stored in Postgres in the `instance_settings` table. Secret settings are encrypted with `BIGRAG_MASTER_KEY` and redacted on read.
...
The UI shows common controls first and keeps raw setting keys, source metadata, live-apply markers, and rarely changed limits in Advanced controls. Legacy Settings deep links such as `/settings?tab=storage` and `/settings?tab=vector_store` still open the relevant Settings panel, while `/settings?tab=models`, `/settings?tab=search`, and `/settings?tab=chat` redirect to `/models?tab=settings`.
