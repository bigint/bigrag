...
| Rate limits | Login/setup windows, per-email/per-IP limits, upload file/byte quotas, and raw vector API count/size limits. |
| Ingestion | Upload limits, upload-session limits, upload-session UI concurrency, OCR toggle, Docling subprocess timeout, ingestion workers, vector batch size. |
