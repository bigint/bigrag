...
| `BIGRAG_QDRANT_REQUIRED` | Fail startup if Qdrant cannot be reached. | `false` |
| `BIGRAG_MASTER_KEY` | Fernet key for encrypted provider secrets and instance-setting secrets. Required for production. | — |
...
| Area | Examples |
|------|----------|
| Security | CORS origins, trusted proxies, session cookie policy, provider URL allow-lists, private-network escape hatches. |
...
| Search | Qdrant `ef`, embedding concurrency, collection/query cache TTLs. |
...
| Retention | Query log, access log, webhook delivery, progress snapshot, upload-session history, and audit retention windows. |
