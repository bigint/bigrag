...
Run background jobs with `bigrag-worker`. The worker reads the same `BIGRAG_` environment and accepts `--processes`, `--threads`, `--queues`, and `--config` for deployment-specific concurrency and TOML bootstrap. The worker parent runs database migrations once before spawning Dramatiq processes, and child processes skip migration bootstrap. The local dev script and Docker Compose default to `BIGRAG_WORKER_PROCESSES=5` and `BIGRAG_WORKER_THREADS=8`.
