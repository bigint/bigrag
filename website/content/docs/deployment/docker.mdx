...
```bash
BIGRAG_API_IMAGE=yoginth/bigrag-api:latest \
BIGRAG_UI_IMAGE=yoginth/bigrag-ui:latest \
docker compose up -d --no-build
```

Starts the bigRAG API, worker, admin UI, Postgres, and Redis. Turbopuffer runs as the managed vector, keyword, and hybrid search backend.
</Step>
<Step>
### Verify
...
- When hacking from a checkout, plain `docker compose up -d` builds local API and UI images.
- When deploying published images, set the image variables shown above and use `--no-build`.

```yaml
services:
  bigrag-api:
    image: yoginth/bigrag-api:latest
    ports:
      - "4000:4000"
    volumes:
      - bigrag_data:/data
