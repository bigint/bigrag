...
services:
  bigrag-api:
    image: yoginth/bigrag-api:<release-tag>
    ports:
      - "4000:4000"
    volumes:
      - bigrag_data:/data
  ...
  bigrag-ui:
    image: yoginth/bigrag-ui:<release-tag>
    ports:
      - "3000:3000"
    environment:
...
Uploads, webhook deliveries, and S3 sync all depend on `bigrag-worker`. If the admin UI shows `bigrag-worker is offline`, start or restart the worker service, then confirm `/v1/stats` reports `"workers": { "online": true }` with a recent `heartbeat_at`.
