...
Text logs are colored by default. Every API request logs `request_start`,
`response_start`, and `request_complete`; unhandled failures log
`request_failed`. Request logs include method, path, sanitized query params,
sanitized headers, client IP, route, endpoint/action context, status,
