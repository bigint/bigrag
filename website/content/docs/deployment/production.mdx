...
- [ ] Backup strategy: configure readable S3/R2 backups from `/settings?tab=backups`, protect that bucket as sensitive, and separately keep encrypted infrastructure snapshots for Postgres, Turbopuffer exports, Redis, and the local staging volume.
...
The admin UI creates backup-only full-instance exports from `/settings?tab=backups`. The destination is S3-compatible — AWS S3, Cloudflare R2, and MinIO all work. Use a bucket or prefix dedicated to backups. The endpoint URL should be the provider root, such as `https://ACCOUNT.r2.cloudflarestorage.com` for Cloudflare R2; bucket names and backup prefixes stay in their own fields. Saving backup settings probes bucket listing permissions before persisting the destination.

**What's included:**

- Postgres rows, vector-store points and payloads, and logs — all in plain JSON, JSONL, and SQL.
...
Uploads, backups, webhook deliveries, and S3 sync all depend on `bigrag-worker`. If the admin UI shows `bigrag-worker is offline`, start or restart the worker service, then confirm `/v1/stats` reports `"workers": { "online": true }` with a recent `heartbeat_at`.
