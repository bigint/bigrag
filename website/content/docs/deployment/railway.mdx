---
title: Railway
description: Deploy bigRAG to Railway with managed Postgres, Redis, local ingestion staging, and Turbopuffer search.
---
...
Check the `Worker` logs and confirm `/v1/stats` shows a recent worker heartbeat. The worker must have the same Postgres, Redis, master-key, Turbopuffer, embedding, and `BIGRAG_UPLOAD_DIR` mount as `API`.

</Accordion>

<Accordion title="Uploads fail before ingestion starts">

Confirm `BIGRAG_UPLOAD_DIR` points to a writable local directory that the API and worker both mount.
