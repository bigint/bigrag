...
| **Uploads** | Railway Bucket for temporary ingestion staging, referenced as `Uploads` |
...
The template uses a Railway Bucket as S3-compatible temporary ingestion staging:
...
Do not use local upload volumes for the split Railway template. API and worker services need to read the same staged files while ingestion is active, so object storage is the shared temporary boundary. Staged originals are deleted after terminal processing, cancellation, or document deletion.
...
<Accordion title="Uploads fail before ingestion starts">
