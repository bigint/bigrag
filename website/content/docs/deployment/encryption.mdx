...
Readable backups created from Settings → Backups are intentionally not client-side encrypted. They export decrypted provider settings, embedding vectors, logs, chats, Qdrant payloads, and raw uploaded files so operators can inspect the backup directly. Put the backup bucket behind strict IAM/R2 token policy, bucket encryption, private networking where available, lifecycle retention, and access logging.
