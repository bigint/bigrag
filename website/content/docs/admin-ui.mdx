...
The `/connectors` page lists configured S3/R2 sources across collections. Collection connector pages show source rows inline and open source creation in a modal; they own per-source credentials, schedule changes, manual sync, source deletion, and sync-job progress through `/v1/connectors/s3/*`. Cloudflare R2 endpoint URLs set the connector signing region to `auto`.
