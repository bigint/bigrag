...
Collection creation keeps tenant-filter enforcement and JSON metadata-schema validation inside Advanced safeguards. Most collections can leave these disabled; enable tenant filtering only when one collection contains data for multiple customers, and enable metadata validation only when uploads need a fixed metadata shape.
