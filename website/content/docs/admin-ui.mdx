...
Submitted operator forms use TanStack Form for client-side draft state, validation, resets, and submit handling while keeping shared Atelier UI controls. This covers setup, login, collection creation and search, eval runs, API keys, MCP servers, account password changes, runtime settings, embedding presets, webhook creation, and connector credential forms. Login fields keep visible labels and short placeholders for the required credentials.
...
| `/login` | Email + password login for admins and members. Already-authenticated sessions are sent back to the requested admin route. |
