---
title: Python SDK
description: Async Python client for the rag.computer API with full type hints and zero config.
---
...
async def main():
    async with RagComputer(api_key="ragc_sk_…") as client:  # minted at /v1/admin/api-keys
        # Create a collection
...
```python
client = RagComputer(
    api_key="...",                  # or set RAG_COMPUTER_API_KEY env var
...
The client reads `RAG_COMPUTER_API_KEY` from the environment if `api_key` is not passed.
...
| `client.chat` | Generated answers, streaming, and conversation CRUD |
...
followup = await client.chat.create({
    "conversation_id": answer["conversation"]["id"],
...
# Conversation CRUD
chats = await client.chat.list(limit=20)
detail = await client.chat.get(chats["conversations"][0]["id"])
...
```python
from rag_computer import (
    RagComputerError,        # base for all errors
