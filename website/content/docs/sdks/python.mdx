...
await client.admin.settings.test({"values": {"vector_store_provider": "qdrant"}})
await client.admin.settings.update({"values": {"session_cookie_secure": True}})
await client.admin.settings.reset({"keys": ["session_cookie_secure"]})
