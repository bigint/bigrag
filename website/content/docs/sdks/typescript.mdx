...
const client = new RagComputer({
  apiKey: process.env.RAG_COMPUTER_API_KEY!, // ragc_sk_... minted via /v1/admin/api-keys
...
|--------|---------|-------------|
| `apiKey` | `RAG_COMPUTER_API_KEY` env var | `ragc_sk_…` key minted at [`/v1/admin/api-keys`](/docs/api-reference/api-keys). Omit for session-cookie auth (browser contexts). |
...
const chats = await client.chat.list({ limit: 20 });
const detail = await client.chat.get(chats.conversations[0].id);
await client.chat.update(detail.conversation.id, { title: "Policy Q&A" });
await client.chat.delete(detail.conversation.id);
```
...
client.admin.users.update(userId, { display_name?, role?, password? })
...
client.admin.apiKeys.update(keyId, { name?, active?, scopes?, collection? })
...
`RagComputerError` > `APIError` > specific errors:
