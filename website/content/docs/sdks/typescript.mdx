...
client.collections.createEventToken(name)
client.collections.streamEvents(name, { token? })    // AsyncIterable of collection events (SSE)
```
...
client.queries.multiQuery({
  query, collections, top_k?, filters?, min_score?, search_mode?, rerank?, multimodal?
})
...
<Accordion title="Admin realtime streams">
