...
<Callout type="info">
  The endpoint uses the MCP Python SDK 2 server in stateless HTTP mode with JSON responses. Every request carries the bearer key and is dispatched through the bigRAG REST API, so auth and scope rules apply uniformly.
</Callout>
...
For `streamable-http`, `--port` sets the server bind port. The stdio transport ignores it.
